...
/**
 * Include Livefyre PHP library
 * @see http://answers.livefyre.com/developers/libraries/
 */
...
$rpx_api_key = trim( file_get_contents( "./api_key" ) );
...
//          ),
    ...
    */
    ...
    /**
     *  Livefyre authentication
     *  Note that this only has been set up for Facebook
     */
    
    $network = Livefyre::getNetwork("demo-accounts-int-0.fyre.co", "networkKeyhere");
    $expires = time()+7*60*60;
    preg_match('/id=[0-9]+/',$auth_info['profile']['identifier'], $fbId);
    $userId = "facebook" . str_replace("id=","",$fbId[0]);
    $token = $network->buildUserAuthToken($userId, $auth_info['profile']['displayName'], $expires);
    setcookie('lfLogin', $token, $expires);
    
    header('Location: http://engage.livefyre.com/');
